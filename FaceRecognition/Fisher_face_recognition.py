...
# Create the text file for storing the scores
#result_file = open("result_fisher.txt", "w")
id_name = ""
prev_id = 0
same_id_count =0
    ...
    for(x,y,w,h) in faces:
        ...
        for (ex, ey, ew, eh) in eyes:
            ...
            if (confidence>40 and confidence<100):
                ...
                id_name = profile[1] if profile is not None else "unknown"
                confidence = "  {0}%".format(round(confidence)) #100-80 =20%
                if (profile!=None):
                    id_name=profile[1]
                    print(id)
                    #with open('result_fisher.txt', 'a') as file:
                        #file.write(f'{id_name}, {confidence}\n')
                    if id == prev_id:
                        same_id_count += 1
                    else:
                        prev_id = id
                    ...
                    if same_id_count == 3:
                        print("The last three id's were the same")
                        same_id_count = 1
